...
export class NostrService {
    ...
    try {
      ...
      if (replyToEventId) {
        ...
        if (pending) {
          ...
          console.error(`Matched payment for ${pending.unicityId} via replyToEventId`);
        }
      }
      ...
      if (!pending) {
        for (const [key, p] of this.pendingPayments) {
          if (p.userPubkey === senderPubkey) {
            pending = p;
            pendingKey = key;
            console.error(`Matched payment for ${p.unicityId} via sender pubkey`);
            break;
          }
        }
      }
  ...
  async sendPaymentRequest(
    ...
    console.error(
      `Sent payment request to ${unicityId} for amount ${this.config.amount} (eventId: ${eventId.slice(0, 16)}...)`
    );
